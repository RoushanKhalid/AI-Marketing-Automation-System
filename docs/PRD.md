...
The AI Marketing Automation System is a Python-based backend service that enables users to create, schedule, and automatically dispatch AI-generated marketing campaigns. It uses the **Groq API** for LLM-powered text generation and **Pollinations.AI** for image URL generation — both free to use. Campaigns are persisted in SQLite and exposed via a FastAPI REST API. SMS delivery is simulated via structured console output.

---

## 2. Goals

- Allow users to create marketing campaigns with a name, prompt, phone number, and scheduled time.
- Automatically generate compelling marketing text using Groq (`llama3-8b-8192`).
- Automatically generate a relevant image URL using Pollinations.AI (no API key required).
- Dispatch campaigns at their scheduled time and simulate SMS output to console.
- Expose all functionality via a clean REST API.
- Store all campaign data persistently in SQLite.
...
| Term | Definition |
|---|---|
| **Campaign** | A marketing job: name, prompt, phone, schedule_time, status |
| **Campaign_Store** | SQLite-backed persistence layer |
| **Text_Generator** | Calls Groq API to produce marketing copy |
| **Image_Generator** | Constructs a Pollinations.AI image URL |
| **Scheduler** | Background thread that polls and dispatches due campaigns |
| **SMS_Simulator** | Prints formatted dispatch output to console |
| **Logger** | App-wide structured logging (console + rotating file) |
| **Prompt** | Natural-language instruction describing desired marketing content |
| **Schedule_Time** | `YYYY-MM-DD HH:MM:SS` datetime for campaign dispatch |
| **Status** | Campaign lifecycle state: `pending` → `processing` → `sent` / `failed` |

---

## 5. Campaign Data Model

Each campaign contains the following fields:

| Field | Type | Constraints |
|---|---|---|
| `campaign_id` | integer | Auto-generated, unique, primary key |
| `campaign_name` | string | Non-empty, max 255 characters |
| `prompt` | string | Non-empty, max 2000 characters |
| `phone` | string | E.164 format: `+` followed by 7–15 digits |
| `schedule_time` | datetime | Format: `YYYY-MM-DD HH:MM:SS` |
| `status` | enum | One of: `pending`, `processing`, `sent`, `failed` |
...
4. THE Text_Generator SHALL use the Groq-hosted chat completion model `llama-3.1-8b-instant` for all text generation requests.
